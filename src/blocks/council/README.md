...
**no cloud API keys** to function: the roster is seeded with local Ollama
...
councilors + 1 chair, all `provider: "ollama"`, so the block works with zero
configuration:
...
| GET | `/council/models` | Live models AEON can reach right now (Settings roles + endpoint registry + live cloud env keys + Ollama `/api/tags`). |
...
None are required — the seeded roster runs entirely on local Ollama. These
...
- `OLLAMA_HOST` — override for the Ollama daemon (default `http://127.0.0.1:11434`); `OLLAMA_MODEL` is used as a last-resort fallback if `/api/tags` can't be reached.
