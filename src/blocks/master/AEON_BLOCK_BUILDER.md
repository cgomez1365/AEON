...
2. **`index.jsx`** — default-export exactly one React component. Relative `fetch()` only (`/api/<id>/...`), with `credentials: 'same-origin'` — the sign-in is an HttpOnly cookie the browser sends for you; add no auth header of your own. Send `x-aeon-self-reported: 1` on every call whose failure you show inline, so AEON's global red banner does not repeat it (a 4xx, 501 or 503 you explain stays yours; any other 5xx still banners). Text fields that can hold more than a line are growing `<textarea>`s; long values wrap. No secrets, no `VITE_` secrets, no hardcoded host or port. Import React, `lucide-react` and `src/kernel` only — an import from `src/components` (the aurora library included) is a HIGH lint finding today and promote refuses the block.
3. **`api/<id>.cjs`** (optional) — `module.exports = (deps) => router`. The router is mounted at `/api` (and `/block/<id>`), so **every route path starts with `/<id>/`**: `router.get('/<id>/status')` answers `/api/<id>/status`; a bare `'/status'` would answer `/api/status` and collide with any other block that did the same. `deps` carries only what the manifest declares: `blockStorage` when `filesystem` is `read`/`write`; `kernelLLM` when `ai: true`. Expose `GET /<id>/widget` returning JSON if the manifest has a `widget` section. Never compute a storage path; use `deps.blockStorage.readJSON/writeJSON(...)` — writes are atomic, and a file that will not parse is moved aside (`<name>.unreadable-<time>`) before you get your fallback, so a read → change → write cannot erase it. No `:param` segments in route paths (`?id=` instead; the boot proof calls declared paths literally). When an AI route has no model assigned, answer 424 with a sentence saying so.
